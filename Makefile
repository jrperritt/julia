...
ifeq ($(OS), Darwin)
	...
	env JULIA=$(DESTDIR)$(private_libdir)/libjulia.$(SHLIB_EXT) \
		$(build_bindir)/stringreplace $${strings -t x - $$JULIA | grep "sys.$(SHLIB_EXT)$$" | awk '{print $$1;}' } "$(private_libdir_rel)/sys.$(SHLIB_EXT)" 256 "$(call cygpath_w,$$JULIA)"
	env JULIA=$(DESTDIR)$(private_libdir)/libjulia-debug.$(SHLIB_EXT) \
		$(build_bindir)/stringreplace $${strings -t x - $$JULIA | grep "sys-debug.$(SHLIB_EXT)$$" | awk '{print $$1;}' } "$(private_libdir_rel)/sys-debug.$(SHLIB_EXT)" 256 "$(call cygpath_w,$$JULIA)"
endif
